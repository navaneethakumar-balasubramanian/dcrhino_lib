...
def qc_plot_v2(qc_plot_input, out_filename, data_date, client_project_id,
               two_way_travel_time_ms=None, peak_search_interval_ms=None):
    """
    Modified from V2 to support index plotting; Will temp nix y, z
    TODO: Update with vertical, tangential, radial labels
    TODO: Update ticklabels (ARE WROING)
    WARNING: needs colourbar and label update
    """
    ...
    #<Get inputs and reshape where appropriate>
    trace_array_dict = qc_plot_input.trace_array_dict
    peak_ampl_x_ndx = qc_plot_input.peak_ampl_x_ndx
    peak_mult_x_ndx = qc_plot_input.peak_mult_x_ndx

    lower_num_ms = qc_plot_input.lower_number_ms
    upper_num_ms = qc_plot_input.upper_number_ms
    trace_array_dict['x'] = np.flipud(trace_array_dict['x'])
    trace_array_dict['y'] = np.flipud(trace_array_dict['y'])
    trace_array_dict['z'] = np.flipud(trace_array_dict['z'])
    #</Get inputs and reshape where appropriate>
    ...
    fig, ax = plt.subplots(nrows=4, sharex=True, figsize=(11,8.5))
    header_plot(ax[0], X, qc_plot_input, out_filename)
    plt.subplots_adjust(right=10.5)
   ...
   #<sort out yticks every 5 ms>
    ...
    ax[2].plot(X, peak_ampl_x_ndx, label='ampl')
    ax[2].plot(X, peak_mult_x_ndx, label='mult')
    ax[2].legend()

    ax[3].plot(X, peak_ampl_x_ndx- peak_mult_x_ndx, label='diff')
    plt.tight_layout()
    if colourbar_type=='all_one':
        cbaxes = fig.add_axes([0.01, 0.1, 0.007, 0.8])
        cb = plt.colorbar(heatmap1, cax = cbaxes)
