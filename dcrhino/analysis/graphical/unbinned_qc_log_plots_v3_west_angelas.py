...
class QCLogPlotInput(object):
    ...
    @property
    def vertical_primary_peak_amplitude(self):
        return self.df['vertical_primary_peak_amplitude']
    @property
    def vertical_primary_peak_sample(self):
        return self.df['vertical_primary_peak_sample']
    @property
    def amplitude_ratio(self):
        return self.df['vertical_multiple_peak_amplitude'] / self.vertical_primary_peak_amplitude

    @property
    def amplitude_ratio_sample(self):
        return self.df['vertical_multiple_peak_sample'] / self.df['vertical_primary_peak_sample']
    @property
    ...
    def primary_wavelet_width(self):
        return (self.df['vertical_primary_zero_crossing_after'] - self.df['vertical_primary_zero_crossing_prior'])
    @property
    def multiple_wavelet_width(self):
        return (self.df['vertical_multiple_zero_crossing_after'] - self.df['vertical_multiple_zero_crossing_prior'])

    @property
    def primary_wavelet_width_sample(self):
        return self.df['vertical_primary_zero_crossing_after_sample'] - self.df['vertical_primary_left_trough_time']

    @property
    def arrival_time_diff(self):
        return self.df['vertical_multiple_peak_time'] - self.df['vertical_primary_peak_time']
    @property
    def arrival_time_diff_sample(self):
        return self.df['vertical_multiple_peak_time_sample'] - self.df['vertical_primary_peak_time_sample']

    @property
    def pseudo_ucs_sample(self):
        return np.sqrt(self.vertical_primary_peak_sample)
    @property
...
def ucs_panel(ax, qc_plot_input, x_limits=[None, None], sample_or_poly='sample'):
    ...
    if sample_or_poly == 'sample':
        ...
        data = np.sqrt(qc_plot_input.vertical_primary_peak_amplitude)
    #pdb.set_trace()
...
def primary_times_panel(ax, qc_plot_input, x_limits=[None, None], sample_or_poly='sample'):
    if sample_or_poly == 'sample':
        peak_time = 1000*qc_plot_input.df['vertical_primary_peak_time_sample']
        zx1 = 1000*qc_plot_input.df['vertical_primary_zero_crossing_prior_sample']
        zx2 = 1000*qc_plot_input.df['vertical_primary_zero_crossing_after_sample']
        #x_limits = [4., 8., ]
    elif sample_or_poly == 'poly':
        peak_time = 1000*qc_plot_input.vertical_primary_peak_time
        zx1 = 1000*qc_plot_input.df['vertical_primary_zero_crossing_prior']
        zx2 = 1000*qc_plot_input.df['vertical_primary_zero_crossing_after']
    #pdb.set_trace()
...
#        pseudodensity_xlim_min = None; pseudodensity_xlim_max = None; i_ax += 1
    ...
    def plot_depth_old_mess(self, qc_plot_input, multi_mode=False):
        """
        """
        #depth = qc_plot_input.depth
        project_id = 'west_angelas'
        ...
        ar_xlim_min = -0.1; ar_xlim_max = 0.9; i_ax = 0
        ratio_panel(ax[i_ax], qc_plot_input, x_limits=[ar_xlim_min, ar_xlim_max])

        rc_xlim_min = 0.; rc_xlim_max = 1.5; i_ax += 1
        reflection_coefficient_panel(ax[i_ax], qc_plot_input, x_limits=[rc_xlim_min, rc_xlim_max])
        ...
        twt_ms = 1000.0 * twt
        twt_xlim_min = twt_ms - 2.0; twt_xlim_max = twt_ms + 7.0; i_ax += 1
        delay_panel(ax[i_ax], qc_plot_input, x_limits=[twt_xlim_min, twt_xlim_max])

        #primary_width_xlim_min = 0.; primary_width_xlim_max = 3.0; i_ax += 1
        primary_width_xlim_min = None; primary_width_xlim_max = None; i_ax += 1
        primary_pseudovelocity_panel(ax[i_ax], qc_plot_input,
                                     x_limits=[primary_width_xlim_min, primary_width_xlim_max])

#        multiple_width_xlim_min = None; multiple_width_xlim_max = 4.0; i_ax += 1
#        multiple_pseudovelocity_panel(ax[i_ax], qc_plot_input,
#                                     x_limits=[multiple_width_xlim_min, multiple_width_xlim_max])
...
#        prime_times_xlim_min = None; prime_times_xlim_max = None; i_ax += 1
#        primary_times_panel(ax[i_ax], qc_plot_input,
#                            x_limits=[prime_times_xlim_min, prime_times_xlim_max])
        plt.savefig(full_out_file)
        #plt.show()
        plt.clf()


    def plot_time(self, qc_plot_input):
        """
        """
        #depth = qc_plot_input.depth
        project_id = 'west_angelas'
        full_out_file = qc_plot_input.plot_meta['time_log_filename']
        ensure_dir(os.path.dirname(full_out_file))
        if os.path.isfile(full_out_file):
            return
        plt.rcParams['figure.figsize'] = [30, 40]
        ...
        #pdb.set_trace()
        well_log_panel_time_plot(ax[0], qc_plot_input.amplitude_ratio,
                            qc_plot_input.component_trace_index, 'Mult/Peak', '(Unitless)',
                            x_limits=[ar_xlim_min, ar_xlim_max])
        ...
        well_log_panel_time_plot(ax[1], qc_plot_input.arrival_time_diff,
                            qc_plot_input.component_trace_index, 'multtime-primarytime', '(seconds)',
                            x_limits=[twt_xlim_min, twt_xlim_max])
