...
class RockPropertiesLogPlotter():
    ...
    def plot(self, plot_input):
        ...
        #pdb.set_trace()
        start_datetime_str = plot_input.observer_row.datetime_start.strftime("%B %d, %Y")
        ...
        output_dir = os.path.join(plot_input.data_level_path, 'rock_properties'
                                  '{}'.format(plot_input.observer_row.bench), '{}'.format(plot_input.observer_row.pattern))
        ensure_dir(output_dir)
        full_out_filename = os.path.join(output_dir, out_fname+'.png')
        #pdb.set_trace()
        plt.savefig(full_out_filename)
        #plt.show()
        plt.clf()
