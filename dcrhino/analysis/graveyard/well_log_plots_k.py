...
#        df = df.drop(df[((df.peak_mult_x / df.peak_ampl_x) < 0.01) | ((df.peak_mult_x / df.peak_ampl_x) > 0.6)].index)
        ...
        for dummy_hole_id, df_per_hole in df.groupby('dummy_hole_id'):
            ...
            title_line1 = 'RHINo Logs, Mont Wright, Quebec, Ca, {}'.format(df_per_hole['datetime'].min().strftime("%B %d, %Y"))
            ...
            plt.legend()
            plt.xlabel('(GPa)', fontsize=label_fontsize)
            ...
            plt.legend()
            plt.xlabel('(MPa)', fontsize=label_fontsize)
...
#            ax[2].yaxis.grid(True,'major',linewidth=2)
            ...
            ax[2].tick_params(axis='y',labelleft=False)
            ax[2].legend(loc=1)
            ax[2].set_xlabel('Density (g/cc)', fontsize=label_fontsize)
            ax[2].set_yticks(np.arange(math.floor(df_per_hole.depth.min()), math.ceil(df_per_hole.depth.max()), 1))
