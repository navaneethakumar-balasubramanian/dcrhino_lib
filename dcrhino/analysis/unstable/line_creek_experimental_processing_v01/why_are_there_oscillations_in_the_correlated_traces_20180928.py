...
project_label = 'line_creek'
project_label = 'MOUNT_MILLIGAN_MINE'
project_dir = os.path.join(home, 'data/datacloud/rhino_process_pipeline_output/', '{}'.format(project_label))
ssx_id = 5451;#ssx_id = 5208
ssx_id = 5206

#hole_id = '793-MR_77-23531'
...
process_run_directory = os.path.join(project_dir, '{}'.format(ssx_id), '{}'.format(int(sampling_rate)))
...
#    hole_id = 23831
    hole_string = '793-MR_77-{}'.format(hole_id)
    data_dir = os.path.join(process_run_directory, hole_string)
    filebase = '{}_filtered_correlated_traces.npy'.format(component)
    ...
    center_sample = int(sampling_rate/2) + int(sampling_rate/20)

    plt.plot(data[:,center_sample]);
    plt.xlabel("trace index (time in seconds)")
    plt.ylabel("Amplitude of non-normalized correlated traces")
    plt.title('{} {} {}Hz, ssx{}'.format(filebase, hole_string, sampling_rate, ssx_id))
    plt.show()
    pdb.set_trace()
