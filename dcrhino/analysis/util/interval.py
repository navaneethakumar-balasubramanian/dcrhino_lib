"""
Source https://github.com/internetarchive/analyze_ocr/blob/master/interval.py
Liscence Not Specified
...
class Interval(object):
    ...
    def _get_start_time(self, **kwargs):
        """
        ...
        if isinstance(self.lower_bound,datetime.datetime):
            ...
            starttime = self.lower_bound.strftime(time_format)
            if ms:
...
#<Add ons to original module start here>
#knk 20180824;
...
def generate_interval_list(startTime, endTime, delta):
    """
    generic function used for specifying timeintervals to load
    probably place in gmi.core.interval
    ...
    while startTime < endTime:
        ...
        upper_bound=startTime+delta, lower_closed=True, upper_closed=False))
...
def merge_interval_list(interval_list):
    """
    Can think of a stack of unassembled parts.  Put the first part in your
    ...
    right to your left hand and contine ...

    KEY: This assumes the interval list is sorted.  If the intervals are not sorted
    then you need to use set union.  This is way faster though.
    """
    new_interval_list = []
    if len(interval_list) > 0:
        left_hand_event = interval_list.pop(0)
    else:
        return new_interval_list

    for i_ivl in range(len(interval_list)):
        right_hand_event = interval_list.pop(0)
        if left_hand_event.overlaps(right_hand_event):
            left_hand_event = left_hand_event.join(right_hand_event)
        else:
            new_interval_list.append(left_hand_event)
            left_hand_event = right_hand_event
