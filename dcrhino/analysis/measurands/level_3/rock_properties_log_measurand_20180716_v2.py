...
from dcrhino.analysis.graphical.rock_properties_log_plots_v2 import RockPropertyPlotInput
...
class RockPropertiesLogMeasurand(SpatiallySampledMeasurand):
    ...
    def id_string(self):
        ...
        """
        id_string = '{}_{}'.format(self.label, self.parent_measurands[0].label)
        return id_string
...
#        basename = self.id_string + '_' + data_key.observer_row.unique_hole_data_id(data_key.mount_point) + '.' + self.extension
    ...
    def _make_from_parents(self, data_key, expected_delay=None):
        ...
        dt = 1./data_key.parent_data_key.sampling_rate
        parent_data_key = data_key.parent_data_key#.digitizer_date_data_key
        #parent_filename = parent_measurand.expected_filename(parent_data_key)
        #parent_label = self.parent_measurands[0].label
        #pdb.set_trace()
        output_filename = self.expected_filename(data_key)
        output_directory = os.path.dirname(output_filename)
        ensure_dir(output_directory)
        binned_df = parent_measurand.load(data_key)
...
#                                                 150.0)
    ...
    def generate_rock_properties_plot_input(self, df, data_key=None, mount_point=None):
        ...
        rp_input.hole_start_time = data_key.master_row.row.time_start.to_pydatetime()
        rp_input.observer_row = data_key.master_row
        rp_input.project_id = self.project_id
