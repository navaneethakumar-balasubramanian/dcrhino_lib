...
interpolated_data.to_csv("Interpolated_4.6_"+MWD_FILE_NAME,index=False,header=True)
print("Finished Interpolating.  Begin Segy creation at "+ str(datetime.now()))
#Create a new obspy segy stream
stream = Stream()
trace_data = np.zeros((SAMPLING_RATE,4),dtype=np.float32)
...
def Add_Ensemble_to_Stream(stream,trace_data,trace_time,trace_number,ensemble,SAMPLING_RATE,f,interpolated_row):
    ...
    for t in range(1,4):
        ...
        trace.stats.segy.trace_header.number_of_samples_in_this_trace = SAMPLING_RATE+1 # 115-116 - 20000 (samples) 
            ...
            if first_sample_of_second == True:
                if microseconds < int(1000000/SAMPLING_RATE):
                    ...
                    if(len(interpolated_data[interpolated_data["time_start"]==ensemble_time.datetime]))>0:
                        interpolated_row = interpolated_data[interpolated_data["time_start"]==ensemble_time.datetime.strftime]
                    else:
                        interpolated_row = empty_row
...
stream.stats.binary_file_header.number_of_samples_per_data_trace_for_original_field_recording = SAMPLING_RATE+1
