...
if __name__ == "__main__":
    ...
    argparser.add_argument('-p', '--path', help="Path for the input files")
    ...
    if(int(float(args.trace_length)*int(args.sample_rate))>32767):
    ...
    splits = int(1.0/float(args.trace_length))

    if(int(args.sample_rate)%splits != 0):
        print("{} samples can't be evenly split in {} second(s) intervals".format(args.sample_rate,args.trace_length))
        sys.exit(0)
    ...
    df.initTraceLength(float(args.trace_length))
