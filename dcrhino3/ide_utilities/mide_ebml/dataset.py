...
class EventList(Transformable):
    ...
    def __init__(self, parentChannel, session=None, parentList = None):
        ...
        #self.noBivariates = False #NRN default was False
        self.noBivariates = df.NOBIVARIATES
...
#===============================================================================
    ...
    def exportSEGY(self, outFileObj, Rhino, start=0, stop=-1, step=1, subchannels=True,
                      callback=None, callbackInterval=0.01, timeScalar=1,
                      raiseExceptions=False, dataFormat="%.6f",delimiter=",",
                      useUtcTime=False, useIsoFormat=False, headers=False,
...
#TODO: Clean the list of parameters that are not needed
            ...
            sr = int(round(sr))
            t0 = datetime.now()
            segy = df.SEGY(sr,Rhino)
            print(segy.stream)

            try:
                for num, evt in enumerate(_self.iterSlice(start, stop, step, display=display)):
                    #pdb.set_trace()
                    segy.addSample(formatter(evt),_self.session.utcStartTime)
                    if callback is not None:
                        if getattr(callback, 'cancelled', False):
                            callback(done=True)
                            break
...
#     #        print(sys.exc_info())
    ...
    def exportArray(self, start=0, stop=-1, step=1, subchannels=True,
                      callback=None, callbackInterval=0.01, timeScalar=1,
                      raiseExceptions=False, dataFormat="%.6f",delimiter=",",
                      useUtcTime=False, useIsoFormat=False, headers=False,
                      removeMean="TotalMean", meanSpan=None, display=False):
    #TODO: Clean the list of parameters that are not needed
            """ Export events as an Array
            @param stream: The stream object to which to write CSV data.
            @keyword start: The first event index to export.
            @keyword stop: The last event index to export.
            @keyword step: The number of events between exported lines.
            ...
            try:
                for num, evt in enumerate(_self.iterSlice(start, stop, step, display=display)):
                    #pdb.set_trace()
                    row = formatter(evt).split(',')
                    pressure.append([df.CreateSampleTime(float(row[0]),_self.session.utcStartTime),row[1]])
                    if callback is not None:
                        if getattr(callback, 'cancelled', False):
                            callback(done=True)
                            break
