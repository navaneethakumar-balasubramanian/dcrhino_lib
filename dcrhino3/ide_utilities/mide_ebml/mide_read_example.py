...
class ChannelDataBlockHandler(object):
    ...
    def __call__(self, element):
        ...
        #NRN start = self.fixOverflow(data['StartTimeCodeAbsMod'])
        #NRN end = self.fixOverflow(data['EndTimeCodeAbsMod'])
        start = self.fixOverflow(data['StartTimeCodeAbs'])
        end = self.fixOverflow(data['EndTimeCodeAbs'])
...
if __name__ == "__main__":
    ...
    #NRN argparser = argparse.ArgumentParser(description= \
    #NRN    "MIDE IDE Parser Example. Writes separate CSV files for the "
    #NRN    "accelerometer and pressure/temperature channels.")
    #NRNargparser.add_argument('-o', '--output', help= \
    #NRN   "The path to which to save the .CSV files. Defaults to the same path as "
    #NRN   "the source file.")
    #NRN argparser.add_argument('source', help="The source .IDE file to convert.")

    #NRNargs = argparser.parse_args()
    #NRN source = args.source
    source = "test.IDE"
    savePath =""
    #NRN savePath = args.output if args.output else os.path.dirname(source)
