...
def exportNPY(events, serial_number,ideFileObj,resampling_rate, **kwargs):
    """ Wrapper for SEGY export
    """
    #pdb.set_trace()
    return events.exportNPY(serial_number,ideFileObj,resampling_rate, **kwargs)
...
def ideExport(ideFileObj,channels=None,resampling_rate=3200, **kwargs):
...
##                    outFileObj.addDirToPath("run_"+outFileObj.TimeStamp)#This line is to create time stamped folder for each conversion run
                print("  Exporting Channel %d (%s) to %s..." % (ch.id, ch.name, "numpy file"))
                events = ch.getSession()#This gets a session that already exists, so it won't change the noBivariates flag at all
                numSamples += exporter(events, sensor_serial_number,ideFileObj,resampling_rate,callback=updater, **exportArgs)[0]
#            else:
...
#    argparser.add_argument('-l', '--trace_length', help="Trace Length", default='1')
    ...
    #argparser.add_argument('-db', '--database', help="Database to save raw_data", default='test')
...
#        sys.exit(0)
    ...
    # resampling_rate = int(args.sample_rate)
    ...
    if os.path.isdir(path):
        ...
        # sourceFiles = glob.glob(path+"*.IDE")
    else:
