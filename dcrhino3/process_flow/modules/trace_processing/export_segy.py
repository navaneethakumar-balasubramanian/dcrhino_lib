...
from dcrhino3.models.segy_trace_header import define_obspy_trace_header  # This module needs to remain here in order to
# redefine the trace headers
...
class ExportSEGYModule(BaseTraceModule):
    ...
    def save_as_segy(self, row):
        ...
        for t in range(3):
            ...
            trace.stats.segy.trace_header.time_basis_code = 4  # It is set to UTC.  If it is not the case, have to
            # update this value
            ...
            trace.stats.segy.trace_header.sensor_distance_to_source = float(
                self.global_config.sensor_distance_to_source)  # Calculated from the top of the Top Sub
            trace.stats.segy.trace_header.sensor_position = float(
                self.global_config.sensor_position)  # Distance from the sensor to the top of the shocksub
            trace.stats.segy.trace_header.sensor_distance_to_shocksub = float(
                self.global_config.sensor_distance_to_shocksub)
            ...
            trace.stats.segy.trace_header.drill_string_total_length = float(
            ...
            try:
                ...
                logger.warning("Could not convert hole_name to float.  Using dummy name of 999.999")
                trace.stats.segy.trace_header.hole_id = 999.999
            ...
            try:
                ...
                logger.warning("This file did not have the acceleration columns")
