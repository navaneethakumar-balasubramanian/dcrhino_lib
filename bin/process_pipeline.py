...
def header_plot(ax, X, peak_ampl_y,peak_ampl_z,peak_mult_x,peak_ampl_x, plot_title, peak_amplitude_linewidth = 0.2):
    """
    """
...
def qc_plot(output_path,plot_title,axial,tangential,radial,ts_array,lower_num_ms=-5,upper_num_ms=30,cmap_string='jet',colourbar_type='all_one'):
...
end_depth_column = 'computed_elevation'
...
#rig_id = str(metadata.rig_id)
#mine_name = str(metadata.mine_name)
#sensor_serial_number = str(metadata.sensor_serial_number)

accelerometer_max_voltage = float(f1.attrs['PLAYBACK/accelerometer_max_voltage'])
#output_sampling_rate = int(f1.attrs['COLLECTION/output_sampling_rate'])
#trace_length = float(f1.attrs['COLLECTION/trace_length_in_seconds'])
#deconvolution_filter_duration = float(f1.attrs['PROCESSING/deconvolution_filter_duration'])
#num_taps_in_decon_filter = num_taps_in_decon_filter(global_config.output_sampling_rate,global_config.deconvolution_filter_duration)
#trapezoidal_bpf_corner_1 = float(f1.attrs['PROCESSING/trapezoidal_bpf_corner_1'])
#trapezoidal_bpf_corner_2 = float(f1.attrs['PROCESSING/trapezoidal_bpf_corner_2'])
#trapezoidal_bpf_corner_3 = float(f1.attrs['PROCESSING/trapezoidal_bpf_corner_3'])
#trapezoidal_bpf_corner_4 = float(f1.attrs['PROCESSING/trapezoidal_bpf_corner_4'])
#trapezoidal_bpf_duration = float(f1.attrs['PROCESSING/trapezoidal_bpf_duration'])
#min_lag_trimmed_trace = float(f1.attrs['PROCESSING/min_lag_trimmed_trace'])
#max_lag_trimmed_trace = float(f1.attrs['PROCESSING/max_lag_trimmed_trace'])
#pdb.set_trace()
#ideal_timestamps = np.arange(0,global_config.output_sampling_rate*trace_length)*(1.0/global_config.output_sampling_rate)
#sensor_axial_axis =int(metadata.sensor_axial_axis)
#sensor_tangential_axis =int(metadata.sensor_tangential_axis)
...
#primary_window_halfwidth_ms = 2.0
#multiple_window_search_width_ms = 3.126
#sensor_distance_to_source = metadata.sensor_distance_to_source
#n_samples = 640

#axial, tangential, radial, ts_array = get_axial_tangential_radial_traces(
#       int(h5_helper.min_ts), int(h5_helper.max_ts), h5_helper.data_xyz, h5_helper.ts, h5_helper.sensitivity_xyz,debug_file_name=os.path.join('', ''))
...
#axial_traces = [None] * interval
#tangential_traces = [None] * interval
#print (len(extracted_features_list))
#radial_traces = [None] * interval
#idx = 0
