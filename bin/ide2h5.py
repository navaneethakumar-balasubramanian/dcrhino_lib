...
def exportH5(events, serial_number, ideFileObj, resampling_rate, config, time_offset, max_file_size_in_sec, **kwargs):
    """ Wrapper for H5 export
    """
    return events.exportH5(serial_number, ideFileObj, resampling_rate, config=config, time_offset=time_offset,
                           max_file_size_in_sec=max_file_size_in_sec, **kwargs)
...
def ideExport(ideFileObj, config, channels=None, resampling_rate=3200, time_offset=0, max_file_size_in_sec=3600,
              **kwargs):
    ...
    for ch in exportChannels:
       ...
       #Ch 70 = Quaternions
        try:
            ...
            logger.info("  Exporting Channel %d (%s) to %s..." % (ch.id, ch.name, "H5 file"))
            ...
            numSamples += exporter(events, sensor_serial_number, ideFileObj, resampling_rate,
                                   config=config, time_offset=time_offset, max_file_size_in_sec=max_file_size_in_sec,
...
if __name__ == "__main__":
    ...
    argparser.add_argument('-TO', '--time_offset', help="Time Offset for clock differences", default=0, type=float)
    ...
    argparser.add_argument('-fs', '--max_file_size_in_seconds', type=int, help="Maximum file size in seconds",
                           default=3600)
    ...
    max_file_size_in_sec = args.max_file_size_in_seconds
    # if args.bvr == 'True':
    noBvr = True
    # else:
    #     noBvr = False
    ...
    if os.path.exists(args.path):
        ...
        logger.error("Input path does not exist.  Try a different path")
        sys.exit(0)
    ...
    try:
        ...
        logger.info("Conversion complete! Exported %s samples in %s (%s samples/sec.)" % (totSamp, tstr, sampSec))
        sys.exit(0)
        ...
        logger.error("*** An unexpected %s occurred. Is source an IDE file?" % err.__class__.__name__)
