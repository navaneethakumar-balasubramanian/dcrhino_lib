  ...
  >
    <v-card>
      <v-toolbar dark color="primary">
        ...
        <v-toolbar-title
          >Comparing {{ holes_info.length }} process</v-toolbar-title
        >
      </v-toolbar>
      ...
      <v-container grid-list-md fluid text-xs-center>
        ...
        <v-layout row wrap v-if="holes_info">
          <v-flex
            xs12
            v-for="hole_info in holes_info"
            :key="hole_info.processed_hole_id"
          >
            <v-subheader class="grey--text text--darken-4">{{
              hole_info.output_folder_name
            }}</v-subheader>
            <v-img
              :src="get_image(hole_info.images[0], hole_info.processed_hole_id)"
            ></v-img>
